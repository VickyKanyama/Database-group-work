...
VALUES(1,1,300,'pending'),
      ...
      (2,2,700,'complete'),
      (4,4,250,'complete');
SELECT p.purchase_ID,m.medicine_name,p.supplier_name,p.purchase_date,p.quantity,m.price,p.status ,(m.price*p.quantity) AS total price FROM purchase p
...
ALTER TABLE purchase CHANGE COLUMN status TO purchase_status ENUM('pending', 'complete') NOT NULL;
UPDATE status SET purchase_status="complete" WHERE m.medicine_ID=1;
